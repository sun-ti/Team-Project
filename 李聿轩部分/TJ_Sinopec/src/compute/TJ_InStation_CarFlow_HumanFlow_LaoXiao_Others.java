...
public class TJ_InStation_CarFlow_HumanFlow_LaoXiao_Others extends Util{
	...
	public void compute(String tablename,String[] times) {
		...
		//	6.卸油车的信息统计;
		tongji_xieyou(stationid,t1,t2,year,month,day);
	...
	private void checkTableExist(String stationid){
	        ...
	        if(list.size()==0){
	            ...
	            sql="create table "+tablename+" (" +
	                ...
	                "tongji_kehu_fenzhandian varchar(255)," +
	                "tongji_kehu_zonggongsi varchar(255),"+ 
	                "datetime varchar(20)," +
	                "year int," +
	                "month int,"+ 
	                "day int," +
	        ...
	        //	检测卸油的表;
	        String xieyou	=	"tongji_"+stationid+"_xieyou";
	        sql      		= 	"select TABLE_NAME from INFORMATION_SCHEMA.TABLES where TABLE_SCHEMA='"+dbname+"' and TABLE_NAME='"+xieyou+"'";
	        list     		= 	baseUtil.select(sql);
	        
	        if(list.size()==0) {
	        	
	        	//	进行表单的创建;
	        	sql="create table "+xieyou+" ("+ 
	        	"autoid bigint primary key auto_increment,"+ 
    			"xieyou_id bigint,"+ 
	        	//	配送单id编号;
	        	"xql varchar(20),"+ 
	        ...
	        //	检测异常的表;
	        String yichang =	"tongji_"+stationid+"_yichang";
	        sql      	   = 	"select TABLE_NAME from INFORMATION_SCHEMA.TABLES where TABLE_SCHEMA='"+dbname+"' and TABLE_NAME='"+yichang+"'";
	        list     	   = 	baseUtil.select(sql);
	        
	        if(list.size()==0) {
	        	
	        	//	进行表单的创建;
	        	sql="create table "+yichang+" ("+ 
	        	"autoid bigint primary key auto_increment,"+ 
    			"yichang_id bigint,"+ 
	        	"psd_id varchar(20),"+ 
    			"car_fact varchar(20),"+ 
	...
	private void tongji_CarFlow_HumanFlow(String stationid,String t1,String t2,String year,String month,String day) {
		...
		if(list.size()!=0) {
			...
			for(String[] items:list) {
				...
				String item=items[0];
				//	获取同车牌入站时间;
				sql						 	= "select min(a.jiankong_id), max(a.jiankong_id) from "+jiankong+" a,shebei_shexiangtou b,zhandian_shexiangtou c where a.deviceip=b.shexiangtou_ip and b.shexiangtou_id=c.shexiangtou_id and c.stationid='"+stationid+"' and b.kind="+TAG_KIND1+" and b.shexiangtou_fun like '%入口%' and a.plate='"+item+"' and a.jiankong_id between "+t1+" and "+t2;
				ArrayList<String[]> temps	= baseUtil.select(sql);

				if(temps.size()!=0) {
					String 			temp 	= temps.get(0)[0];
...
//			System.out.println("车牌:"+plate+" | 同辆车的时间差:"+dis1+"天 | 最后一次出现与当前时间差:"+dis2+"天");
	...
	private void tongji_client_fenlei(String stationid,String t1,String t2,String year,String month,String day) {
		...
		sql					=	"select count(distinct a.plate) "+ 
		
								"from "+jiankong+" a, shebei_shexiangtou b, zhandian_shexiangtou c, guanli_kehu_type d "+
								
								"where a.deviceip=b.shexiangtou_ip and "+ 
								
								"b.shexiangtou_id=c.shexiangtou_id and "+ 
								
								"c.stationid='"+stationid+"' and "+ 
								
								"b.kind="+TAG_KIND1+" and "+ 
								
								"b.shexiangtou_fun like '%入口%' and "+ 
								
								"a.plate=d.plate and "+ 
								
								"d.type_id="+TAG_KIND1_CLIENT+" and "+ 
								
								"a.jiankong_id between "+t1+" and "+t2;
		...
		sql					=	"select count(distinct a.plate) "+ 
								...
								"d.type_id="+TAG_KIND2_CLIENT+" and "+ 
		...
		sql					=	"select count(distinct a.plate) "+ 
								...
								"d.type_id="+TAG_KIND3_CLIENT+" and "+ 
		...
		sql					=	"select count(distinct a.plate) "+ 
								...
								"d.type_id="+TAG_KIND4_CLIENT+" and "+ 
		...
		sql					=	"select count(distinct a.plate) "+ 
								...
								"from "+jiankong+" a, shebei_shexiangtou b, zhandian_shexiangtou c, "+guanli+" d "+
	...
	//	统计卸油车的相应信息;
	private void tongji_xieyou(String stationid,String t1,String t2,String year,String month,String day) {
		//	SQL语句;
		String 				sql			=	"";
		//	数据查询集合;
		ArrayList<String[]> list		=	null;
		...
		//	从监控表中获取相应的监控车牌;
		sql						  = "select distinct a.plate,a.datetime from "+jiankong+" a,shebei_shexiangtou b,zhandian_shexiangtou c where a.deviceip=b.shexiangtou_ip and b.shexiangtou_id=c.shexiangtou_id and c.stationid='"+stationid+"' and b.kind="+TAG_KIND1+" and b.shexiangtou_fun like '%入口%' and plate!='无' and a.jiankong_id between "+t1+" and "+t2+" order by jiankong_id";
		...
		if(list.size()!=0) {
			...
			for(String[] items:list) {
				...
				//	监控获得的车牌-去重;
				String 	 	plate_jiankong	= 	items[0];
				String		date_fact		=	items[1];
				
				//	查询当前车牌是否属于卸油车;
				sql							=	"select count(*) from "+xieyouche+" where plate='"+plate_jiankong+"'";
				...
				if(count!=0) {
					...
					//	查询此车是否在加油单中;
					sql		=	"select xql,depot_code,psd_id,oils_id,ps_date from "+peisongdan+" where cp_no='"+plate_jiankong+"' and nodeno='"+stationid+"' and ps_date like '%"+date+"%'";
					temps 	= 	baseUtil.select(sql);
					
					//	1.属于配送单的放入到卸油表中;
					if(temps.size()!=0) {
						for(String[] datas:temps) {

							long     xieyou_id   = System.currentTimeMillis();
							String   xql		 = datas[0];
							String   depot_code  = datas[1];
							String   psd_id	     = datas[2];
							String   cp_no	     = plate_jiankong;
							String   car_fact    = cp_no;
							String   oils_id	 = datas[3];
							String   ps_date	 = datas[4];
			    			String   state	     = "0";
			    			
				        	//	进行SQL更新操作;
							sql				 = 
							"insert into "+xieyou+" (xieyou_id,xql,depot_code,psd_id,cp_no,car_fact,oils_id,ps_date,date_fact,year,month,day,state) values("+ 
							""+xieyou_id+",'"+xql+"','"+depot_code+"','"+psd_id+"','"+cp_no+"','"+car_fact+"','"+oils_id+"','"+ps_date+"','"+date_fact+"',"+year+","+month+","+day+","+state+")";
					...
					//  2.不属配送单的放入到异常表中;
						...
						long	yichang_id=	System.currentTimeMillis();
						String	car_fact  = plate_jiankong;
						String  state	  = "0";
						
						sql				  = "insert into "+yichang+" (yichang_id,car_fact,datetime,state) values ("+yichang_id+",'"+car_fact+"','"+date_fact+"',"+state+")";
